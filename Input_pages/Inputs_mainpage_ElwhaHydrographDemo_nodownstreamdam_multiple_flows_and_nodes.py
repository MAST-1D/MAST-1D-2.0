...
inputs.TmultHighFlow = 150 # Number of timesteps for days with flow above High Flow Timestep Threshold
...
inputs.HighFlowTimestepThreshold = 300 # threshold discharge above which hydrograph runs use a shorter timestep.
...
inputs.LoadFactor = [1.,0.8]  #  List of upstream sediment feed (in fraction of equilibrium capacity)
...
inputs.DischargeFiles = ['Qfile15Sep2011pres','Qfile15Sep2011pres_c'] # Names 
...
# List of variables to save at regular intervals (as strings of clsNode attributes)
...
'Floodplain.GSD.F[0]', 'Floodplain.GSD.D84', 'CumulativeTotBedMaterialFeed', 'SLatSourceAv[-1]', \
'CobbleMobility', 'WidenRate', 'NarrowRate', 'PointBarSubsurfaceGSD.D50']

# List of variables for daily output (as strings of clsReach attributes)    
inputs.DailyOutputVars=['Node[10].Load.QsjTot[0]',
    'Node[10].ActiveLayer.GSD.D50',
    'Node[0].DC.Qw[0]',
    'Node[-1].DC.Qw[0]',
    'Node[10].Bc',
    'Node[10].BarPavingRatio',  
