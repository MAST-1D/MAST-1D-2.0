...
class clsProbScale(mscale.ScaleBase): # Katie add 'cls' to the name
    """
    ...
    def __init__(self, axis, **kwargs):
        ...
        upper = kwargs.pop("upper", 99.5) #Default to an upper bound of 98% # Katie changed
    ...
    def set_default_locators_and_formatters(self, axis):
        ...
        major_ticks = np.asarray([.2,1,5,20,40,60,75,90,98,99.5]) #removed a couple ticks to make it look nicer # Katie changed to match USGS PeakFQ plot
        major_ticks = major_ticks[np.where( (major_ticks >= self.lower) & (major_ticks <= self.upper) )]

        # Katie comment out minor ticks
        #minor_ticks = np.concatenate( [np.arange(.2, 1, .1), np.arange(1, 2, .2), np.arange(2,20,1), np.arange(20, 80, 2), np.arange(80, 99.5, 1)] ) # Katie modify here
    ...
    class ProbTransform(mtransforms.Transform):
        ...
        def transform_non_affine(self, a):
            ...
            ppf = norm.ppf(masked/100., self.mu, self.sigma)*100 # Katie change masked to masked/100.
            #print cdf
            return ppf
        ...
        def inverted(self):
            ...
            """
            return clsProbScale.InvertedProbTransform(self.lower, self.upper, self.mu, self.sigma)
    ...
    class InvertedProbTransform(mtransforms.Transform):
        ...
        def transform_non_affine(self, a):
            ...
            inverse = norm.cdf(a, self.mu, self.sigma) # Katie change a/100 to a
            return inverse

        def inverted(self):
            return clsProbScale.ProbTransform(self.lower, self.upper)
